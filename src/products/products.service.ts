...
export class ProductsService {
  constructor(
    ...
    @InjectModel('Productstock') private readonly productstockModel: Model<any>,
  ...
  public async getProductsByPagination(
    page: number,
    limit: number,
  ): Promise<CommonResponseModel> {
    const products = await this.productModel
    ...
    return {
      ...
      response_data: { products, totalProducts, paginationCount },
    };
  }
  ...
  public async saveProduct(
    ...
    if (user.role !== 'Admin') {
      return {
        response_code: HttpStatus.UNAUTHORIZED,
        response_data: 'You are not allowed to create product',
      };
    }
    ...
    if (response._id) {
      ...
      return {
        response_code: HttpStatus.BAD_REQUEST,
        response_data: 'Could not save product',
      };
    }
  }
  ...
  public async updateProductRecord(
    ...
    productId: string,
    productData: ProductsDTO,
    productstockData: ProdStockDTO,
  ): Promise<CommonResponseModel> {
    if (user.role !== 'Admin') {
      return {
        response_code: HttpStatus.UNAUTHORIZED,
        response_data: 'You are not allowed to update product',
      };
    }
    if (!productId) {
      return {
        response_code: HttpStatus.BAD_REQUEST,
        response_data: 'Object ID is required',
      };
    }
    ...
    return {
      response_code: HttpStatus.OK,
      response_data: 'Product updated successfully',
    };
  }
  // checks if product is linked to deal of not
  public async checkProductDealLinking(
    productId: string,
  ): Promise<CommonResponseModel> {
    const dealData = (await this.dealsModel.find({
      products: productId,
    })) as Array<DealsDTO>;
    if (dealData.length > 0) {
      return {
        response_code: HttpStatus.FOUND,
        response_data: 'Product you want to delete is linked to a deal',
      };
    } else {
      return {
        response_code: HttpStatus.OK,
        response_data: 'No linking found. You can delete the product',
      };
    }
  }
  ...
  public async deleteProduct(
    ...
    if (checkIfPresent.length > 0) {
      return {
        response_code: HttpStatus.BAD_REQUEST,
        response_data:
          'Cannot delete this product since it has a reference in orders',
      };
    }
    return {
      response_code: HttpStatus.OK,
      response_data: 'Product deleted successfully',
    };
  }
  public async stockUpdateOne(
    user: UsersDTO,
    id: string,
    productstockData: ProdStockDTO,
  ): Promise<CommonResponseModel> {
    const res = await this.productstockModel.findByIdAndUpdate(
      id,
      productstockData,
    );
  ...
  public async ProductDataCustomize(cart, products) {
    for (let item of cart.cart) {
      ...
      const productIndex = products.findIndex(
        val => val._id.toString() == item.productId.toString(),
      );
      if (productIndex === -1) {
      } else {
  ...
  public async productInfo(
    ...
    if (query && query.userId && product) {
      ...
      let cart = await this.cartModel.findOne(
        { user: query.userId, isOrderLinked: false },
        'cart',
      );
  ...
  public async homePage(query: any): Promise<CommonResponseModel> {
    ...
    let products = await this.productModel
      .find(
        { status: 1 },
        'title filePath imageUrl isDealAvailable delaPercent category variant averageRating',
      )
      .limit(limit)
      .sort({ createdAt: -1 });
    ...
    const categories = await this.categoryModel
      .find(
        { status: 1 },
        'title filePath imageUrl category isSubCategoryAvailable',
      )
      .limit(catLimit)
      .sort({ createdAt: -1 });
    const dealsOfDay = await this.dealsModel
      .find(
        { status: 1 },
        'name filePath delaPercent imageUrl delalType category product',
      )
      .limit(topLimit)
      .sort({ createdAt: +1 });
    const topDeals = await this.dealsModel
      .find(
        { status: 1, topDeal: true },
        'name filePath imageUrl delaPercent delalType category product',
      )
      .limit(topLimit)
      .sort({ createdAt: -1 });
    return {
  ...
  public async homePageTopDeal(): Promise<CommonResponseModel> {
    const topDeals = await this.dealsModel
      .find(
        { status: 1, topDeal: true },
        'name imageUrl filePath delaPercent delalType category product',
      )
      .sort({ createdAt: -1 });
    return {
      response_code: HttpStatus.OK,
  ...
  public async homePageCategory(): Promise<CommonResponseModel> {
    const categories = await this.categoryModel
      .find({ status: 1 }, 'title filePath imageUrl category')
      .sort({ createdAt: -1 });
    return {
      response_code: HttpStatus.OK,
  ...
  public async homePageProduct(query: any): Promise<CommonResponseModel> {
    ...
    if (query && query.page) {
      products = await this.productModel
        .find(
          { status: 1 },
          'title filePath imageUrl category isDealAvailable delaPercent variant averageRating',
        )
  ...
  public async searchProduct(
    ...
    let products = await this.productModel.find(
      { title: { $regex: search_key, $options: 'i' }, status: 1 },
      'title filePath imageUrl isDealAvailable delaPercent category variant averageRating',
    );
    if (query && query.userId && products && products.length) {
      let cart = await this.cartModel.findOne(
  ...
  public async getFilterData(
    ...
    if (
      search_key === 'HIGHTOLOW' ||
      search_key === 'Hightolow' ||
      search_key === 'High To Low' ||
      search_key === 'high to low' ||
      search_key === 'High to low'
    ) {
    ...
    if (
      search_key === 'LOWTOHIGH' ||
      search_key === 'Low To High' ||
      search_key === 'lowtohigh' ||
      search_key === 'low to high' ||
      search_key === 'Low to high'
    ) {
  ...
  public async findBrand(id: string): Promise<CommonResponseModel> {
    var products = await this.productModel.find({ brand: id });
    return {
      response_code: HttpStatus.OK,
  ...
  public async getProductsBySubCategory(
    ...
    let products = await this.productModel
      .find(
        { subcategory: subCategoryId, status: 1 },
        'title imageUrl filePath category subcategory isDealAvailable delaPercent variant averageRating',
      )
      .sort({ createdAt: -1 });
    if (query && query.userId && products && products.length) {
      let cart = await this.cartModel.findOne(
